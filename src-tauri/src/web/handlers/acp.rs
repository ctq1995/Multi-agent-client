...
pub async fn acp_get_agent_status(
    ...
    let result = acp_commands::acp_get_agent_status_inner(params.agent_type, &db)
        .await
...
pub async fn acp_connect(
    ...
    if let registry::AgentDistribution::Npx { package, .. } = meta.distribution {
        if !acp_commands::is_cmd_available(package) {
            return Err(AppCommandError::task_execution_failed(format!(
...
pub async fn acp_preflight(
    ...
    let result = acp_commands::acp_preflight(params.agent_type)
        .await
        .map_err(|e| AppCommandError::task_execution_failed(e.to_string()))?;
    Ok(Json(result))
}
